              ...
              var set = function set(attrName, value) {
                switch (attrName) {
                  ...
                  case 'metaData':
                    ...
                    break;
                  default:
                    // File 并非一�? AVObject，不能完全自定义其他属性，所以只能都放在 metaData 上面
      ...
      */
      ...
      module.exports = function (AV) {
        ...
        _.extend(AV.Insight, /** @lends AV.Insight */{
          ...
          /**
           * 开始一�? Insight 任务。结果里将返�? Job id，你可以拿得到的 id 使用
           * AV.Insight.JobQuery 查询任务状态和结果�?
           * @param {Object} jobConfig 任务配置�? JSON 对象，例如：<code><pre>
           ...
           *                   }
           *                  </pre></code>
           *               sql 指定任务执行�? SQL 语句�? saveAs（可选） 指定将结果保存在哪张表里，limit 最�? 1000�?
           * @param {AuthOptions} [options]
           * @return {Promise} A promise that will be resolved with the result
           * of the function.
           */
           ...
           *     仅在云引擎运行环境下有效�?
           *  </em></strong></p>
           * @param {String} event 监听的事件，目前仅支�? 'end' ，表示任务完�?
           * @param {Function} 监听回调函数，接�? (err, id) 两个参数，err 表示错误信息�?
        ...
        AV.Insight.JobQuery.prototype = {
          ...
          /**
           * 查询任务状态和结果，任务结果为一�? JSON 对象，包�? status 表示任务状态， totalCount 表示总数�?
      ...
      (function (process) {
        ...
        var createApiUrl = function createApiUrl(route, className, objectId, method, dataObject) {
          // TODO: 兼容 AV.serverURL 旧方式设�? API Host，后续去�?
          if (AV.serverURL) {
        ...
        var setServerUrlByRegion = function setServerUrlByRegion() {
          ...
          getServerURLPromise = new Promise(function (resolve, reject) {
            // 如果用户�? init 之前设置�? APIServerURL，则跳过请求 router
      ...
      module.exports = function (AV) {
        ...
        AV.User = AV.Object.extend("_User", /** @lends AV.User.prototype */{
           ...
           * @deprecated 如果要判断当前用户的登录状态是否有效，请使�? currentUser.isAuthenticated().then()�?
           * 如果要判断该用户是否是当前登录用户，请使�? user.id === currentUser.id
           * @return (Boolean) whether this user is the current user and is logged in.
           */
          authenticated: function authenticated() {
            console.warn('DEPRECATED: 如果要判断当前用户的登录状态是否有效，请使�? currentUser.isAuthenticated().then()，如果要判断该用户是否是当前登录用户，请使用 user.id === currentUser.id�?');
            return !!this._sessionToken && !AV._config.disableCurrentUser && AV.User.current() && AV.User.current().id === this.id;
          },

          /**
           * 检查该用户的登录状态是否有效，请注意该方法会校�? sessionToken 的有效性，是个异步方法�?
           *
           * @since 2.0.0
          ...
          /**
           * 使用当前使用小程序的微信用户身份注册或登录，成功后用户的 session 会在设备上持久化保存，之后可以使�? AV.User.current() 获取当前登录用户�?
           * 仅在小程序中可用�?
           *
           * @since 2.0.0
           * @return {AV.User}
